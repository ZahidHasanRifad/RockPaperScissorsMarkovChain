...
public class RockPaperScissorsMain {
		...
		if(game.previous != null) {
			...
			for (int i = 0; i < State.values().length; i++) {
				for (int j = 0; j < State.values().length; j++) {
					System.out.print(game.transitionModel[i][j] + "\t");
				}
				System.out.println("\n");
			}
			System.out.println("---------------------------------------");
	...
	private void initialize() throws IOException {
		...
		aiState.setForeground(new Color(0, 0, 0));
		...
		paperButton.setFont(new Font("Tahoma", Font.BOLD, 14));
		...
		scissorsButton.setFont(new Font("Tahoma", Font.BOLD, 14));
		...
		lblHuman.setFont(new Font("Tahoma", Font.BOLD, 14));
		lblHuman.setBounds(134, 36, 64, 14);
		...
		lblComputer.setBounds(458, 36, 82, 14);
